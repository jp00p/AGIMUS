...
async def handle_starboard_reactions(payload: discord.RawReactionActionEvent) -> None:
    ...
    if board_patterns is None:
        board_patterns = generate_board_compiled_patterns(
            config["handlers"]["starboard"]["boards"]
        )
        blocked_channels = get_channel_ids_list(
            config["handlers"]["starboard"]["blocked_channels"]
        )
        boards = get_channel_ids_list(board_patterns.keys())
        high_react_channel_ids = get_channel_ids_list(
            config["handlers"]["starboard"]["high_react_channels"]
        )
    ...
    if (
        payload.channel_id in blocked_channels
        or payload.channel_id in boards
    ...
    ):  # only textchannels work here for now (FUTURE ME: now i'm trying to remember why...)
        return
    ...
    if isinstance(reaction, str):
        logger.info(
            f"{Style.BRIGHT}{Fore.YELLOW}WHOA THE REACTION IS A STRING{Fore.RESET}{Style.RESET_ALL}"
        )
        return
    ...
    for board, match_reacts in board_patterns.items():
                ...
                if hasattr(this_emoji, "name"):
                    ...
                    if match.search(this_emoji.name.lower()) is not None:
                        ...
                        async for user in reaction.users():
                            ...
                            if (
                                user != message.author
                                and user not in message_reaction_people
                            ):
                                total_reacts_for_this_match += 1
                                message_reaction_people.add(
                                    user
        ...
        if (
            total_reacts_for_this_match >= adjusted_react_threshold
            and len(message_reaction_people) >= user_threshold
        ):
            if (
                get_starboard_post(message.id, board) is None
            ):  # checking again just in case (might be expensive)
...
async def add_starboard_post(message, board) -> None:
    ...
    if len(message.attachments) <= 0 and message.content.lower().startswith(
        "https://tenor.com/"
    ):
        return

    await increment_user_xp(
        message.author, 2, "starboard_post", message.channel
    )  # give em that sweet sweet xp first
    ...
    (
        ...
        "" for i in range(6)
    ]  # initialize all the blank strings
    jumplink = (
        f"[View original message]({message.jump_url}) from {message.channel.name}"
    )
    ...
    if len(message.embeds) > 0:
        ...
        message_without_url = (
            message.content.lower().replace(original["url"].lower(), "").strip()
        )
    ...
    star_embed.set_author(name=message.author.display_name, icon_url=author_thumb)

    star_embed.set_footer(text=f"Posted on {date_posted}", icon_url=footer_thumb)
    ...
    logger.info(
        f"{Fore.RED}AGIMUS{Fore.RESET} has added {message.author.display_name}'s post to {Style.BRIGHT}{board}{Style.RESET_ALL}!"
    )
