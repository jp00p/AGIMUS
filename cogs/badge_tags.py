...
class TagButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        associated_tags = db_get_associated_badge_tags(
            self.user_discord_id, self.badge_name
        )
        tag_ids_to_delete = [
            t["id"] for t in associated_tags if t["id"] not in self.view.tag_ids
        ]
        badge_info = db_get_badge_info_by_name(self.badge_name)
        db_delete_badge_tags_associations(
            tag_ids_to_delete, badge_info["badge_filename"]
        )
        if len(self.view.tag_ids):
            db_create_badge_tags_associations(
                self.user_discord_id, self.badge_name, self.view.tag_ids
            )

        new_associated_tags = db_get_associated_badge_tags(
            self.user_discord_id, self.badge_name
        )
        ...
        if len(new_associated_tag_names):
            description = (
                f"**{self.badge_name}** is now tagged with:"
                + "\n\n- "
...
class BadgeTags(commands.Cog):
    ...
    @tags_group.command(name="delete", description="Delete an existing badge tag")
    @option(
        name="tag",
    ...
    @tags_group.command(name="rename", description="Rename an existing badge tag")
    @option(
        name="tag",
    ...
    @option(name="new_name", description="New name for the tag", required=True)
    ...
    @tags_group.command(name="tag", description="Tag one of your badges!")
    @option(
        name="badge",
    ...
    async def tag(self, ctx: discord.ApplicationContext, badge: str):
        ...
        embed = discord.Embed(title=badge, color=discord.Color.dark_purple())
        badge_image = discord.File(
            fp=f"./images/badges/{badge_info['badge_filename']}",
            filename=badge_info["badge_filename"],
        )
        embed.set_image(url=f"attachment://{badge_info['badge_filename']}")

        await ctx.followup.send(
            embed=embed, file=badge_image, view=view, ephemeral=True
        )

    @tags_group.command(
        name="showcase", description="Display a showcase of tagged badges"
    )
    @option(
        name="public",
    ...
    async def showcase(self, ctx: discord.ApplicationContext, public: str, tag: str):
        ...
        badge_images = await generate_paginated_badge_images(
            ctx.author,
            "showcase",
            tagged_badges,
            total_badges_cnt,
        ...
        if not public:
            buttons = [
                pages.PaginatorButton(
                    ...
                    label="   ⬅   ",
                    style=discord.ButtonStyle.primary,
                    disabled=bool(tagged_badges_cnt <= 30),
                    row=1,
                ),
                pages.PaginatorButton(
                    "page_indicator",
            ...
            pages_list = [
                pages.Page(files=[image], embeds=[embed]) for image in badge_images
            ]
            ...
            file_chunks = [
                badge_images[i : i + 10] for i in range(0, len(badge_images), 10)
            ]
