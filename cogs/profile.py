...
def user_photos_autocomplete(ctx: discord.AutocompleteContext):
    user_photos = [
        s["item_name"]
        for s in db_get_user_profile_photos_from_inventory(ctx.interaction.user.id)
    ]
...
def user_stickers_autocomplete(ctx: discord.AutocompleteContext):
    user_stickers = [
        s["item_name"]
        for s in db_get_user_profile_stickers_from_inventory(ctx.interaction.user.id)
    ]
...
def user_styles_autocomplete(ctx: discord.AutocompleteContext):
    user_styles = [
        s["item_name"]
        for s in db_get_user_profile_styles_from_inventory(ctx.interaction.user.id)
    ]
...
class Profile(commands.Cog):
    ...
    profile = discord.SlashCommandGroup(
        "profile", "Commands for displaying and customizing your profile"
    )

    @profile.command(name="display", description="Show your own profile card")
    @option(
        name="public",
    ...
    async def display(self, ctx: discord.ApplicationContext, public: str):
        ...
        logger.info(
            f"{Fore.CYAN}{member.display_name} is looking at their own {Back.WHITE}{Fore.BLACK}profile card!{Back.RESET}{Fore.RESET}"
        )
        ...
        for role in member.roles:
            rolename = (
                role.name.encode("ascii", errors="ignore").decode().strip().lower()
            )
            for rank in ranks:
                if rolename == rank:
        ...
        percent_completed = (
            ((xp - prev_level) * 100) / (next_level - prev_level)
        ) / 100  # for calculating width of xp bar
        ...
        title_font = ImageFont.truetype(
            "fonts/lcars3.ttf", 34
        )  # also subtitle font and score font
        ...
        draw.text(
            (283, 80),
            f"USS HOOD PERSONNEL FILE #{member.discriminator}",
            fill=random.choice(lcars_colors),
            font=level_font,
            align="right",
        )
        draw.text(
            (79, 95),
            f"AGIMUS MAIN TERMINAL",
            fill="#dd4444",
            font=agimus_font,
            align="center",
        )
        ...
        draw.text(
            (250, 418), f"{top_role}", fill="white", font=entry_font, align="left"
        )
        draw.text(
            (250, 478),
        ...
        draw.text(
            (250, 508),
            f"{second_role} / USS Hood NCC-42296",
            fill="white",
            font=entry_font,
            align="left",
        )
        ...
        draw.text(
            (250, 598), f"{user_join}", fill="white", font=entry_font, align="left"
        )
        draw.text(
            (350, 740),
            f"BADGES: {badge_count:03d}",
            fill="black",
        ...
        draw.text(
            (507, 751),
            f"RECREATIONAL CREDITS:",
            fill="black",
            font=big_button_font,
            align="center",
        )
        draw.text(
            (554, 781), f"{score:08d}", fill="black", font=title_font, align="center"
        )
        draw.text(
            (388, 850),
            f"XP: {xp:05d}",
        ...
        screen_glare = Image.open(
            "./images/profiles/template_pieces/lcars/screen-glare.png"
        ).convert("RGBA")
        ...
        padd_frame = Image.open(
            f"./images/profiles/template_pieces/lcars/{frame_filename}"
        ).convert("RGBA")
        ...
        if (
            len(user["stickers"]) > 0
            and user["stickers"][0]["sticker"]
            and user["stickers"][0]["sticker"] != "none"
        ):
            ...
            sticker_image = Image.open(
                "./images/profiles/template_pieces/lcars/sticker-1.png"
            ).convert("RGBA")
            sticker_mask = (
                Image.open("./images/profiles/template_pieces/lcars/sticker-1-mask.png")
                .convert("L")
                .resize(sticker_image.size)
            )
            sticker = (
                Image.open(f"./images/profiles/stickers/{sticker_selection}")
                .convert("RGBA")
                .resize(sticker_image.size)
            )
            sticker_bg = Image.open(
                "./images/profiles/template_pieces/lcars/sticker-bg.png"
            ).convert("RGBA")
            composed = Image.composite(sticker_image, sticker, sticker_mask).convert(
                "RGBA"
            )
            sticker_bg.paste(composed, (0, 0), composed)
            sticker_bg = sticker_bg.rotate(
                random.randint(-6, 6), expand=True, resample=Image.BICUBIC
            )
            ...
            base_bg.paste(
                sticker_bg,
                (sticker_offset + random.randint(-10, 5), 886 + random.randint(-3, 3)),
                sticker_bg,
            )
            ...
            if badge_filename not in [b["badge_filename"] for b in user_badges]:
                ...
                if user["receive_notifications"]:
                    try:
                        ...
                        logger.info(
                            f"Unable to send notification to {ctx.author.display_name} regarding their cleared profile badge, they have their DMs closed."
                        )
            else:
                # Otherwise go ahead and stamp the badge on their profile PADD
                badge_image = (
                    Image.open(f"./images/badges/{badge_filename}")
                    .convert("RGBA")
                    .resize((170, 170))
                )
        ...
        if user["photo"] and user["photo"] != "none":
            ...
            photo_image = Image.open(
                "./images/profiles/template_pieces/lcars/photo-frame.png"
            ).convert("RGBA")
            photo_content = Image.open(
                f"./images/profiles/polaroids/{profile_photo}.jpg"
            ).convert("RGBA")
            ...
            base_bg.paste(
                photo_image,
                (6 + random.randint(-1, 1), 164 + random.randint(-5, 5)),
                photo_image,
            )

        base_w, base_h = base_bg.size
        base_bg = base_bg.resize(
            (int(base_w * 2), int(base_h * 2))
        )  # makes it more legible maybe

        # finalize image
        base_bg.save("./images/profiles/drunkshimodanumber" + str(member.id) + ".png")
        discord_image = discord.File(
            "./images/profiles/drunkshimodanumber" + str(member.id) + ".png"
        )
        await ctx.followup.send(file=discord_image, ephemeral=not public)

    @profile.command(name="set", description="Change your profile settings")
    @option(
        name="tagline",
    ...
    async def set_tagline(self, ctx: discord.ApplicationContext, tagline: str) -> str:
        ...
        if tagline.strip().lower() in ("", "none"):
            ...
            logger.info(
                f"{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}removed their tagline!{Style.RESET_ALL}"
            )
            ...
            logger.info(
                f'{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}changed their tagline{Style.RESET_ALL} to: {Style.BRIGHT}"{tagline}"{Style.RESET_ALL}'
            )
        return msg
    ...
    async def set_badge(self, ctx: discord.ApplicationContext, badge: str) -> str:
        ...
        if remove:
            ...
            logger.info(
                f"{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}removed their profile badge!{Style.RESET_ALL}"
            )
        ...
        logger.info(
            f'{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}changed their profile badge{Style.RESET_ALL} to: {Style.BRIGHT}"{badge}"{Style.RESET_ALL}'
        )
    ...
    async def set_photo(self, ctx: discord.ApplicationContext, photo: str) -> str:
        ...
        if remove:
            ...
            logger.info(
                f"{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}removed their profile photo!{Style.RESET_ALL}"
            )
        ...
        user_photos = [
            s["item_name"]
            for s in db_get_user_profile_photos_from_inventory(ctx.author.id)
        ] + ["None"]
        ...
        logger.info(
            f'{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}changed their profile photo{Style.RESET_ALL} to: {Style.BRIGHT}"{photo}"{Style.RESET_ALL}'
        )
    ...
    async def set_sticker(self, ctx: discord.ApplicationContext, sticker: str) -> str:
        ...
        if remove:
            ...
            logger.info(
                f"{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}removed their profile sticker!{Style.RESET_ALL}"
            )
        ...
        user_stickers = [
            s["item_name"]
            for s in db_get_user_profile_stickers_from_inventory(ctx.author.id)
        ] + ["Default"]
        ...
        logger.info(
            f'{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}changed their profile sticker{Style.RESET_ALL} to: {Style.BRIGHT}"{sticker}"{Style.RESET_ALL}'
        )
    ...
    async def set_style(self, ctx: discord.ApplicationContext, style: str):
        ...
        user_styles = [
            s["item_name"]
            for s in db_get_user_profile_styles_from_inventory(ctx.author.id)
        ] + ["Default"]
        ...
        logger.info(
            f'{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}changed their profile style{Style.RESET_ALL} to: {Style.BRIGHT}"{style}"{Style.RESET_ALL}'
        )
        return f'You\'ve successfully set your PADD style as "{style}"'

    async def set_photo_filter(
        self, ctx: discord.ApplicationContext, filter: str
    ) -> str:
        ...
        if filter not in Profile.filters and filter not in ["random", "none"]:
            ...
            logger.info(
                f'{Fore.CYAN}{ctx.author.display_name}{Fore.RESET} has {Style.BRIGHT}changed their photo filter{Style.RESET_ALL} to: {Style.BRIGHT}"{filter}"{Style.RESET_ALL}'
            )
