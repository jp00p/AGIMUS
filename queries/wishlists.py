...
async def db_get_active_wants(user_discord_id: str, prestige_level: int) -> list[dict]:
  ...
  '''
  async with AgimusDB(dictionary=True) as db:
    return await db.fetch_all(sql, (prestige_level, user_discord_id))
...
async def db_get_wishlist_matches(user_discord_id: str, prestige_level: int) -> list[dict]:
  sql = '''
    WITH
      ...
      partner_owns AS (
        SELECT owner_discord_id AS partner_id,
               badge_info_id
        FROM badge_instances
      ...
      partner_wants AS (
        SELECT user_discord_id AS partner_id,
               badge_info_id
        FROM badge_instances_wishlists
    ...
    SELECT
      ...
      JSON_ARRAYAGG(DISTINCT bi1.badge_name) AS badges_you_want_that_they_have,
      JSON_ARRAYAGG(DISTINCT bi2.badge_name) AS badges_they_want_that_you_have
    FROM partner_owns po
    JOIN partner_wants pw
      ON pw.partner_id = po.partner_id
    JOIN badge_info bi1 ON bi1.id = po.badge_info_id
    JOIN badge_info bi2 ON bi2.id = pw.badge_info_id
    GROUP BY po.partner_id;
  '''
  async with AgimusDB(dictionary=True) as db:
    return await db.fetch_all(sql, (user_discord_id, user_discord_id, prestige_level, prestige_level))
