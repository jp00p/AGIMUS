...
try:
    ...
    logger.error(
        f"{Fore.RED}<! ERROR: Trivia Failed on Import, unable to register cog. !> {e}{Fore.RESET}"
    )
    pass
...
from handlers.xp import (
    handle_event_creation_xp,
    handle_message_xp,
    handle_react_xp,
    increment_user_xp,
)
...
logger.info(
    f"{Style.BRIGHT}{Fore.LIGHTRED_EX}ENVIRONMENT VARIABLES AND COMMANDS LOADED{Fore.RESET}{Style.RESET_ALL}"
)
logger.info(
    f"{Style.BRIGHT}{Fore.LIGHTMAGENTA_EX}CONNECTING TO DATABASE{Fore.RESET}{Style.RESET_ALL}"
)
seed_db()
ALL_USERS = dict.fromkeys(
    get_all_users(), True
)  # used for registering new users without a db lookup
logger.info(
    f"{Style.BRIGHT}{Fore.LIGHTGREEN_EX}DATABASE CONNECTION SUCCESSFUL{Fore.RESET}{Style.RESET_ALL}"
)
...
async def on_message(message: discord.Message):
    ...
    try:
        ...
        logger.info(
            f"{Fore.RED}<! ERROR: Encountered error in process_commands !> {e}{Fore.RESET}"
        )
    ...
    try:
        ...
        logger.error(
            f"{Fore.RED}<! ERROR: Encountered error in saving message task !> {e}{Fore.RESET}"
        )
    ...
    try:
        ...
        logger.error(
            f"{Fore.RED}<! ERROR: Encountered error in handlers !> {e}{Fore.RESET}"
        )
        logger.error(traceback.format_exc())
    ...
    try:
        ...
        logger.error(
            f"{Fore.RED}<! ERROR: Failed to process message for xp !> {e}{Fore.RESET}"
        )
        logger.error(traceback.format_exc())
    ...
    if any(message.content.lower().startswith(x) for x in ["computer:", "agimus:"]):
        logger.info(
            f"Attempting to process {Fore.CYAN}{message.author.display_name}{Fore.RESET}'s command: {Style.BRIGHT}{Fore.LIGHTGREEN_EX}{message.content}{Fore.RESET}{Style.RESET_ALL}"
        )
        try:
...
async def process_command(message: discord.Message):
    ...
    if user_command in config["commands"].keys():
        ...
        if config["commands"][user_command]["enabled"]:
            ...
            access_granted = await perform_channel_check(
                message, config["commands"][user_command]
            )
            logger.info(
                f"Access granted? {Fore.LIGHTGREEN_EX}{access_granted}{Fore.RESET}"
            )
            if access_granted:
                logger.info(
                    f"Firing command: {Fore.LIGHTGREEN_EX}{user_command}{Fore.RESET}"
                )
                try:
            ...
            logger.error(
                f"{Fore.RED}<! ERROR: This function has been disabled: '{user_command}' !>{Fore.RESET}"
            )
    else:
...
async def on_ready():
    try:
        logger.info(
            f"{Back.LIGHTRED_EX}{Fore.LIGHTWHITE_EX} LOGGED IN AS {bot.user} {Fore.RESET}{Back.RESET}"
        )
        logger.info(
            f"{Back.RED}{Fore.LIGHTWHITE_EX} CURRENT ASSIGNMENT: {bot.guilds[0].name} (COMPLEMENT: {len(ALL_USERS)}) {Fore.RESET}{Back.RESET}"
        )
        ...
        logger.info(
            f"{Fore.LIGHTMAGENTA_EX}BOT IS ONLINE AND READY FOR COMMANDS!{Fore.RESET}"
        )
        logger.info(
            f"{Fore.LIGHTRED_EX}CURRENT NUMBER OF STARBOARD POSTS:{Fore.RESET}{Style.BRIGHT} {Fore.BLUE}{number_of_starboard_posts}{Fore.RESET}{Style.RESET_ALL}"
        )
        ...
        await bot.change_presence(
            ...
            activity=discord.Activity(
                name=selected_presence["name"], type=selected_presence["type"]
            ),
        )
    except Exception as e:
...
async def on_typing(channel, user, when):
    ...
    if not ALL_USERS.get(int(user.id)):
        logger.info(
            f"{Fore.LIGHTMAGENTA_EX}{Style.BRIGHT}New User{Style.RESET_ALL}{Fore.RESET}"
        )
