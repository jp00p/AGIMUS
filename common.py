...
def register_user(user):
    with AgimusDB() as query:
        ...
        query.execute(sql, vals)
        logger.info(
            f"{Style.BRIGHT}Registering user to DB:{Style.RESET_ALL} {user.id} {user.display_name} {user.mention}"
        )
    return int(user.id)
...
def win_jackpot(winner, id):
    with AgimusDB() as query:
        ...
        update_this_user = (
            "UPDATE users SET jackpots = jackpots + 1 WHERE discord_id = %s"
        )
        user_vals = (id,)
...
def generate_local_channel_list(client):
    if client.guilds[0]:
        channels = (
            ...
            + client.guilds[0].threads
            + client.guilds[0].voice_channels
            + client.guilds[0].forum_channels
        )
        channel_list = {}
        for channel in channels:
            channel_name = (
                channel.name.encode("ascii", errors="ignore").decode().strip()
            )
            channel_list[channel_name] = channel.id
        # channel_list_json = json.dumps(channel_list, indent=2, sort_keys=True)
        updated_channel_list = deep_dict_update(
            {"channels": config["channels"]}, {"channels": channel_list}
        )
...
def get_emoji(emoji_name: str):
    ...
    if not emoji:
        logger.info(
            f"Emoji: {Fore.LIGHTWHITE_EX}{Style.BRIGHT}{emoji_name}{Style.RESET_ALL}{Fore.RESET} not found, falling back to default bot emoji"
        )
        emoji = random.choice(["🤖", "👽", "🛠"])
    return emoji
...
def run_make_badger():
    result = {"completed": False, "error": False, "version": "", "log": ""}
    try:
