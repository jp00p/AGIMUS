...
mysqldump -h"${DB_HOST}" -u"${DB_USER}" -p"${DB_PASS}" -B "${DB_NAME}" | gzip -9 | s3cmd --host nyc3.digitaloceanspaces.com --region nyc3 put - "s3://${S3_BUCKET_NAME}/${DB_DUMP_S3_PREFIX}/${DB_DUMP_FILENAME_WITH_TIMESTAMP}.gz"
