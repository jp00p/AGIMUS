...
then
    ...
    BACKUP_TO_RESTORE=$(s3cmd --secret_key "${S3_SECRET_KEY}" --access_key "${S3_ACCESS_KEY}" ls "s3://${S3_BUCKET_NAME}/${DB_DUMP_S3_PREFIX}/" | grep -v DIR | sort -rnk2 | head -1 | awk '{ print $4 }')
...
s3cmd --secret_key "${S3_SECRET_KEY}" --access_key "${S3_ACCESS_KEY}" --no-progress get "${BACKUP_TO_RESTORE}" - | gunzip > "${DB_DUMP_FILENAME_WITH_TIMESTAMP}"
